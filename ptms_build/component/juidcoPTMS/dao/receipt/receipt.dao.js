...
class ReceiptDao {
    post = async (req) => {
        ...
        const receipt_no = (0, generateUniqueNo_1.generateReceiptNumber)(data.id);
        const dataWithReceiptNo = await prisma.receipts.update({
            where: { id: data.id },
            data: { receipt_no },
        });
        return (0, generateRes_1.generateRes)(dataWithReceiptNo);
    };
    // ======================== GET RECEIPTS =========================================//
    get = async (req) => {
    ...
    passenger_status = async (req) => {
        const { from_date, to_date } = req.body;
        const date = new Date().toISOString().split("T")[0];
        const qr_func = (condition) => {
            return `
        SELECT COUNT(id)::INT,SUM (amount)::INT,COUNT(id)::INT as no_of_receips FROM receipts ${condition ? condition : `where date = '${date}'`} 
      `;
        };
        let qr = qr_func();
