...
class ReportDao {
    generateReport = async (req) => {
        ...
        let amounts = [];
        let query = "";
        const conductor = ", receipts.conductor_id ";
        function query_fn(extend_query, conductor) {
            return `
        select bus_id, sum(amount)::INT as total_collection ,date, bm.status ${conductor || ""} from receipts 
        ...
        ${extend_query} group by bus_id, date, bm.status ${conductor || ""} order by date ASC
      `;
        }
        query = query_fn("");
        if (conductor_id) {
      ...
      `);
        ...
        return (0, generateRes_1.generateRes)({ result });
    };
    getTotalAmount = async (req) => {
        const { bus_id, conductor_id, from_date, to_date, curr_date } = req.body;
        let query = "";
        function query_fn(extend_query) {
            return `
      ...
      `;
        ...
        if (bus_id && from_date && to_date) {
            query = query_fn(`where bus_id = '${bus_id}' AND date BETWEEN '${from_date}' AND '${to_date}'`);
        }
        //   ------------------------- FILTER BY BUS -----------------------------//
        //   ------------------------- FILTER BY CONDUCTOR-----------------------------//
        if (conductor_id) {
        ...
        if (conductor_id && from_date) {
            query = query_fn(` where conductor_id = '${conductor_id}' AND date = '${from_date}'`);
        }
        if (conductor_id && from_date && to_date) {
            query = query_fn(`where conductor_id = '${conductor_id}' AND date BETWEEN '${from_date}' AND '${to_date}'`);
        }
        //   ------------------------- FILTER BY CONDUCTOR-----------------------------//
    ...
    generateAllReports = async (req) => {
        const { from_date, to_date } = req.body;
        const limit = Number(req.query.limit);
        const page = Number(req.query.page);
        const offset = (page - 1) * limit;
        const qr_func = (condition) => {
            return `
      SELECT
      ...
      `;
        ...
        const qr_func_2 = (conductor_id, condition) => {
            return `
      ...
      `);
        ...
        const [all_conductor] = await prisma.$transaction([
            prisma.$queryRawUnsafe(qr_1, limit, offset),
        ]);
        const all_conductor_data = [];
        if (all_conductor !== null) {
            const promises = all_conductor.map(async (item) => {
                if (from_date && to_date) {
                    qr_2 = qr_func_2(item?.conductor_id, `AND date between '${from_date}' and '${to_date}'`);
                }
                else {
        ...
        return (0, generateRes_1.generateRes)(all_conductor_data);
    };
    //   ------------------------- GET ALL REPORT COLLECITON ----------------------------//
    demographicCount = async (req) => {
        const { from_date, to_date } = req.body;
        const { startOfWeek, endOfWeek } = (0, helper_1.getCurrentWeekRange)();
        const qr_func = (condition) => {
            return `
        	SELECT
            ...
            date
          FROM receipts
         ${condition || `where date between '${startOfWeek}' and '${endOfWeek}'`} group by date
      `;
        };
        let qr_1 = qr_func();
        if (from_date && to_date) {
