...
class ConductorGenerateReportServices {
    ...
    getMonthlyCollection = async (req, res, apiId) => {
        ...
        from 
        receipts
        where conductor_id = ${Number(conductor_id)} and EXTRACT(MONTH FROM date) = ${Number(onlyMonth)} and EXTRACT(YEAR FROM date) = ${Number(onlyYear)}
        `;
...
exports.default = ConductorGenerateReportServices;
